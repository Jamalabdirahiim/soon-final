      ...
      <div className="container mx-auto flex h-20 items-center justify-between px-4 md:px-6">
          ...
          {navLinks.map((link) => (
            <Link
              ...
              className={cn(
                ...
                hasScrolled ? "text-foreground/80" : "text-primary-foreground/80"
              )}
            >
              {link.label}
            </Link>
        ...
        <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
          <SheetTrigger asChild className="md:hidden">
            <Button variant="ghost" size="icon" className={cn(!hasScrolled && "text-white hover:text-white hover:bg-white/10")}>
