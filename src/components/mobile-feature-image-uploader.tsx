...
export function MobileFeatureImageUploader() {
  ...
  const handleUpload = useCallback(async () => {
    if (!file || !firestore) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "Uploader not ready. Please try again.",
      });
      return;
    }
    ...
    try {
        ...
        setDoc(configDocRef, { mobileFeatureImageUrl: dataUrl }, { merge: true })
          .then(() => {
            revalidateHome();
            toast({
                title: "Upload Successful!",
                description: "Your new mobile IPTV image has been applied.",
            });
            setFile(null);
          })
          .catch(error => {
            ...
            const permissionError = new FirestorePermissionError({
              ...
              operation: 'update',
              requestResourceData: { mobileFeatureImageUrl: 'REDACTED_DATA_URL' }
            });
        ...
        toast({
            ...
            description: "Could not process the image. Please check the console for details.",
        });
    } finally {
  ...
  return (
    <div className="w-full">
        <h3 className="font-medium mb-2">Mobile IPTV Image</h3>
        <div 
            {...getRootProps()} 
            className={cn(
        ...
        >
            ...
            {preview ? (
                <div className="relative w-full h-full p-4">
                    <Image src={preview} alt="Mobile IPTV image preview" layout="fill" objectFit="contain" />
                    {!isProcessing && (
